...
![](../image/WavelengthPlots.png)

相反，我们可以把色彩绘制成![](../svg/27.svg)或![](../svg/30.svg)的函数。让我们看看在3D![](../svg/27.svg)空间中绘制的色彩是什么样子的。

![](../image/LinearRGBCube.png)

太棒了!这给了我们一种更广泛的色彩的可视化，而不仅仅是彩虹的光谱色彩。

将其降到两个维度的一个简单方法是为每一对值都有一个单独的图，如下所示：

![](../image/RGBPairPlots.png)
...
![](../image/TriangleSliceRGB.png)

这个立方体的三角形切片具有![](../svg/17.svg)的性质，我们可以用![](../svg/34.svg)作为亮度的粗略逼近。如果我们对这个三角形切片进行自顶向下的观察，我们会得到这样的结果：

![](../image/rgChromaticity1.png)

这种色彩的二维表示称为**色度**。这种特殊的色彩叫做[**Rg色度**](https://en.wikipedia.org/wiki/Rg_chromaticity)。色度为我们提供了与亮度无关的原色比例的信息。

这意味着我们可以有相同的色度在许多不同的强度。

![](../image/rgChromaticity2.png)

我们甚至可以制作一个色度图，其中强度随r和g的变化，以使强度最大化，同时保持**R**、**G**和**B**之间的比率。

![](../image/rgChromaticity3.png)
