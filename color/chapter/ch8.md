...
这些特殊的色彩匹配函数称为![](../svg/21.svg)​、![](../svg/23.svg)​和![](../svg/13.svg)​。

![](../image/cmfs1.png)

这给出了与600nm每个![](../svg/27.svg)​坐标(0.34，0.062，0.00)相关的纯光谱色彩。这是[CIE 1931年RGB色彩空间](https://en.wikipedia.org/wiki/CIE_1931_color_space#CIE_RGB_color_space)中的一个值。

等等，当函数变成负值是什么意思，就像这里？

![](../image/cmfs2.png)

与500nm相关的纯光谱色的![](../svg/27.svg)坐标为(-0.72，0.85，0.48)。那么-0.72到底是什么意思？

事实证明，无论你把红色(700nm)刻度盘设置到什么位置，都不可能匹配500nm的输出光，不管蓝色和绿色刻度盘的值是多少。然而，你可以通过在底部加上红灯来使两面相匹配。

![](../image/ColorMatching3.png)
...
![](../image/ColorMatchingLemon.png)

通过取光谱曲线乘积下的面积和配色函数，我们得到一个![](../svg/27.svg)三重态(1.0，0.8，0.2)唯一识别这种色彩。

而![](../svg/30.svg)色彩空间给了我们一种精确的色彩识别方法，而这个![](../svg/27.svg)色彩空间为我们提供了一种精确的色彩再现方法。但是，正如我们在配色函数中所看到的，任何带有负![](../svg/27.svg)坐标的色彩都不能被实际再现。
